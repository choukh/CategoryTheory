...
现在, 给定任意 S : Subcategory, 我们说以下用 S 构造的范畴 Sub 是 C 的子范畴, 其中 \`1 和 \`2 分别是 projT1 和 projT2 的记法.
...
关于 Sub 的构造, 自然语言解释如下:
- Sub 的对象集 obj 由 C 中那些满足 sobj 的对象组成, 也就是说 Sub 的对象集是 C 的对象集的子集.
...
以上说的集都指搜集 collection. 所谓 "配上" 是指用 existT 构造 sig 的项, 库中使用了 (・;・) 的记法.
